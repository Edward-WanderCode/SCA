...
/**
 * Generate PDF report as Buffer for sending to Telegram
 * Returns the PDF as a Buffer instead of downloading it
 */
export const generateScanPdfBuffer = (scanData: ScanData): { buffer: Buffer; filename: string } | null => {
    try {
        const { source, stats, findings, timestamp, id } = scanData;
        console.log('Generating PDF Buffer for Telegram:', source.name);
        ...
        console.error('PDF Buffer Generation Error:', err);
        return null;
    }
}
